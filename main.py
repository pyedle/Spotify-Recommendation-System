...
spotify_data = pd.read_csv(
    'D:\PY\/3rd Year\/6th Sem\/6th Sem Practical Assignments\Artificial Intelligence Lab\pythonProject\SpotifyFeatures.csv')
spotify_data.head()
...
client_id = '10f3e500b67349068610e6ee531e6be1'
client_secret = '54e02b140d0f4a1588e18e1aad92baa0'
...
playlist_df = generate_playlist_df('Fav', playlist_dic, spotify_data)
...
def visualize_cover_art(playlist_df):
    ...
    for i, url in enumerate(temp):
        ...
        image = io.imread(temp[i], plugin='matplotlib')
        plt.imshow(image)
        plt.xticks([])
        plt.yticks([])
        s = ''
...
def generate_recommendation(spotify_data, playlist_vector, nonplaylist_df):
    ...
    non_playlist['sim'] = cosine_similarity(nonplaylist_df.drop(['track_id'], axis=1).values,
